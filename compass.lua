...
local font = renderCreateFont('tahoma', 7, font_flag.BOLD)
local font_tag = renderCreateFont('Consolas', 14, font_flag.BOLD + font_flag.SHADOW)
...
function main()
    ...
    local cx, cy = 960, 1029
    local compass = true 
    
    sampRegisterChatCommand("cmp_change", function()
		lua_thread.create(function()
			wait(100)
			chat("Переместите компас в желамое место и нажмите {F8C471}ЛКМ{ffffff}, чтобы сохранить местоположение.")
			sampSetCursorMode(2)
	...
	sampRegisterChatCommand("cmp_off", function()
		chat(string.format("Вы изменили состояние отображения компаса на {F8C471}%s{ffffff}.", (not compass)))
		rewrite(string.format("local compass = %s", compass), string.format("local %s = %s", "compass", tostring(not compass)))
	end)
...
function renderCompass(sx, sy, round, bool)
	if isCharDead(playerPed) or not sampIsLocalPlayerSpawned() then return end
	local angle = bool and getCameraHeading() or math.ceil(getCharHeading(playerPed))
	local round_angle = math.round(angle, round)
	
	for index = -6, 6 do
		local current_angle = round_angle + round * -index
		if current_angle < 0 then current_angle = 360 + current_angle 
		elseif current_angle > 360 then current_angle = current_angle - 360 end
		
		local current_direction = direction(current_angle)
		local information = current_direction and current_direction or 360 - current_angle
		local color = current_direction and join_argb(255 - math.upzero(index) * 35, 255, 35, 35) or join_argb(255 - math.upzero(index) * 35, 255, 255, 255)
		
		local center = (sx + 30 * index) + 13.5
		renderFontDrawText(font, information, center - renderGetFontDrawTextLength(font, information) / 2, sy, color)
		
		if index == 0 then 
            renderDrawBox(sx + 30 * index, sy + 13, 27, 3, color)
            local zone = calculateZone()
            if zone then
                local zone = string.upper(zone)
                renderFontDrawText(font, zone, center - renderGetFontDrawTextLength(font, zone) / 2, sy + 17, color)
                --renderFontDrawText(font, point, center - renderGetFontDrawTextLength(font, point) / 2, sy + 30, color)
            end
        else renderDrawBox(sx + 30 * index, sy + 13, 27, 1, color) end
	end
end
...
function math.round(number, r)
	return r * math.floor(number / r)
end

function math.upzero(number)
	return tonumber(string.match(number, '%d+'))
end

function direction(angle)
	local list = {[0] = 'С', [360] = 'С', [180] = 'Ю', [270] = 'В', [90] = 'З', [45] = 'СЗ', [315] = 'СВ', [135] = 'ЮЗ', [225] = 'ЮВ'}
	return list[angle]
end
...
function kvadrat()
    local KV = {[1] = "А", [2] = "Б", [3] = "В", [4] = "Г", [5] = "Д", [6] = "Ж", [7] = "З", [8] = "И", [9] = "К", [10] = "Л", [11] = "М", [12] = "Н", 
        [13] = "О", [14] = "П", [15] = "Р", [16] = "С", [17] = "Т", [18] = "У", [19] = "Ф", [20] = "Х", [21] = "Ц", [22] = "Ч", [23] = "Ш", [24] = "Я",
    }
	...
	if x and y and z then
		...
		y = KV[y]
		if y then return ('%s-%s'):format(y, x) end
	end  return ''
end
